...
class FormBuilder
{
    ...
    private function addCaptchaField(SymfonyFormBuilder $formBuilder, string $name, array $field, Collection $config): void
    {
        ...
        {
            case 'recaptcha_v3':
                if (!$config->has('recaptcha') || !boolval($config->get('recaptcha')['enabled'])) {
            ...
            case 'recaptcha_v2':
                ...
                $this->hasRecaptchaV2Invisible = isset($field['options']['captcha_invisible']) && boolval($field['options']['captcha_invisible']);
            ...
            case 'hcaptcha':
                if (!$config->has('hcaptcha') || !boolval($config->get('hcaptcha')['enabled'])) {
                    // Allow users to simply disable CAPTCHA protection by flipping the flag
                    return;
                }
         ...
         if ($field['type'] == 'submit' && ($this->hasRecaptchaV3 || $this->hasRecaptchaV2Invisible)) {
             ...
             if ($this->hasRecaptchaV3) {
                 $attr = [
                     ...
                     'data-action' => $formName // pass the name of the form as the Action for reCAPTCHA v3
                 ];
             } else if ($this->hasRecaptchaV2Invisible) {
                 $attr = [
                     'class' => 'g-recaptcha',
                     'data-sitekey' => $config['recaptcha']['public_key'],
                     'data-callback' => 'onRecaptchaSubmitted',
             ...
             $field['options'] = array_merge_recursive($field['options'], ['attr' => $attr]);
         }
